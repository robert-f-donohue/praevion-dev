...
class UpgradeHVACMiniSplit < OpenStudio::Measure::ModelMeasure

    def name
        return 'Upgrade HVAC Mini Split'
    end
    ...
    def arguments(model)
        ...
        ms_option = OpenStudio::Measure::OSArgument.makeChoiceArgument('ms_option', choices, true)
        ms_option.setDisplayName('Select Ductless Mini-Split Option')
        ms_option.setDefaultValue('None')
        args << ms_option
    ...
    def run(model, runner, user_arguments)
        ...
        ms_option  = runner.getStringArgumentValue('ms_option', user_arguments)

        # return early if None is selected
        if ms_option == 'None'
            runner.registerInfo('No Mini-Split heat pump added.')
            return true
        end
        ...
        model.getThermalZones.each do |zone|
            ...
            # create a new packaged terminal heat pump (mini-split)
            mini_split = OpenStudio::Model::ZoneHVACPackagedTerminalHeatPump.new(
                model,
            ...
            mini_split.setName("Mini-Split HP - #{zone.name}")

            # add the heat pump to the zone
            mini_split.addToThermalZone(zone)

            # increment the zone counter
            zones_modified += 1
        end

        runner.registerFinalCondition("Installed mini-split heat pumps in #{zones_modified} thermal zones.")
...
UpgradeHVACMiniSplit.new.registerWithApplication
