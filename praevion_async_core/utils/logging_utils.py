...
def archive_run_logs(run_logs_dir: str, archive_base: str):
    ...
    Parameters:
        ...
        archive_base (str): Base directory to store the archive (e.g., 'praevion_async_core/archive/run_logs').
    """
        ...
        if all(col in df.columns for col in obj_cols):
            ...
            for _, group in grouped:
                if len(group) == 1:
                    ...
                    pareto = group[group["pareto_efficient"] == True]
                    if not pareto.empty:
...
def log_optimization_summary_to_csv(csv_path: str, run_label: str, max_evals: int, output_csv_path: str):
    """
    ...
    pareto_df = df[df["pareto_efficient"] == True]
    ...
    obj_ranges = {
        ...
        for col in obj_cols if col in df.columns
    }
